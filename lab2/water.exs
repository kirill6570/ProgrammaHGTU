defmodule Water do
  def run do
    ...
    IO.puts("#{total} #{steps}")
    Enum.each(history, fn row -> IO.puts(Enum.join(row, " ")) end)
  end
  ...
  defp simulate(full, m, total, empty, steps, history) when full > 0 do
    ...
    if new_empty >= m do
      ...
      simulate(exchange, m, new_total, remainder, new_steps + 1, history ++ [row, before_row])
    else
      simulate(0, m, new_total, new_empty, new_steps, history ++ [row])
    end
  end

  defp simulate(0, _, total, empty, steps, history) do
    if empty > 0 do
      {total, 0, steps + 1, history ++ [List.duplicate(0, empty)]}
    else
