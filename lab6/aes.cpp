...
// ОПРЕДЕЛЕНИЯ S-BOX (таблица замен AES)
...
// ОПРЕДЕЛЕНИЯ RCON (раундовые константы)
const unsigned char rcon[11] = { 0x00, 0x01, 0x02, 0x04, 0x08, 0x20, 0x40, 0x80, 0x1b, 0x36, 0x00 };

// Умножение на 2 в поле Галуа GF(2^8)
unsigned char xtime(unsigned char x) {
    return (x << 1) ^ ((x & 0x80) ? 0x1b : 0x00);
}

// Вывод массива байтов в шестнадцатеричном виде
...
// PKCS7 дополнение (до кратности 16)
...
// SubBytes – замена байтов по S-Box
void subBytes(unsigned char state[16]) {
    for (int i = 0; i < 16; ++i) state[i] = sbox[state[i]];
}

// ShiftRows – циклический сдвиг строк влево
...
// AddRoundKey – побайтовый XOR с раундовым ключом
